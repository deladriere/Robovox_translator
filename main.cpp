...
bool displayPresent = false; // Flag to check device presence
...
byte lastSetChannel = 0; // to store the last channel set
...
bool checkI2CDevice(int address)
{
  ...
  if (error == 0)
  {
    ...
    return true; // Device is present at this address
  }
  else
  {
    Serial.println("Display NOT found");
    return false; // No device found at this address
  }
}
...
void updateChannel()
{
  ...
  // Directly read the pin states and combine them
  // Adjust the bit shifts and masks based on the actual pin mappings
  SetChannel |= (!(PORT->Group[PORTA].IN.reg & (1 << 14))) << 3; // Invert Digital Pin 2
  SetChannel |= (!(PORT->Group[PORTA].IN.reg & (1 << 9))) << 2;  // Invert Digital Pin 3
  SetChannel |= (!(PORT->Group[PORTA].IN.reg & (1 << 20))) << 1; // Invert Digital Pin 6
  SetChannel |= (!(PORT->Group[PORTA].IN.reg & (1 << 21)));      // Invert Digital Pin 7

  if (displayPresent && SetChannel != lastSetChannel)
  {
    returnHomeLCD(); // Return to home position (0, 0).
    printDisplayLCD("Robovox Chan.");
    locateCursorLCD(15, 0); // Print at col 8
    char buffer[5];
    itoa(SetChannel, buffer, 10); // Convert int to char*
    printDisplayLCD(buffer);       // Print at col 8

    // LCD.sendString("Robovox Chan.", 0, 0); // Label
    // char buffer[5];
    // itoa(SetChannel, buffer, 10);  // Convert int to char*
    // LCD.sendString(buffer, 15, 0); // Print at col 8
    lastSetChannel = SetChannel;
  }
}
void rot()
{
...
void controlChange(byte channel, byte number, byte value)
{
  ...
  */
  ...
  case 64:
    digitalWrite(MISO, value >> 6); // external carrier
    break;
  }
}
...
void TriggerPhonem(const size_t phonem_idx)
{
  ...
  {
    //   idx = rand() % (kNumPhonems - 1); // disabled for Anthony TODO: add a threshold
  }
  ...
  {
    char line[17];
    snprintf(line, sizeof(line), "%-16s", phonem.label); // pad with spaces to 16 chars
    locateCursorLCD(0, 1);
    printDisplayLCD(line); // Print phonem label on the second line
    // LCD.sendString(line, 0, 1);
  }
...
void handleNoteOn(byte channel, byte pitch, byte velocity)
{
  ...
  // Serial.println();
  analogWrite(RED_LED, velocity << 1);
...
void setup()
{
  ...
  if (displayPresent)
  {
    ...
    printDisplayLCD("Robovox");  
    locateCursorLCD(0, 1);
    printDisplayLCD("Version");
    locateCursorLCD(8,1);
    printDisplayLCD(VERSION);
  }
